    ...
    class YOLO(object):
        ...
        def custom_loss(self, y_true, y_pred):
            ...
            y_pred = tf.reshape(y_pred,
                                (1, tf.shape(y_pred)[1], tf.shape(y_pred)[2], self.nb_box, 4 + 1 + self.nb_class))
            y_true = tf.reshape(y_true,
                                (1, tf.shape(y_true)[1], tf.shape(y_true)[2], self.nb_box, 4 + 1 + self.nb_class))

            cell_x = tf.to_float(
                tf.reshape(tf.tile(tf.range(grid_h * 32), [grid_h * 32]), (1, grid_h * 32, grid_h * 32, 1, 1)))
            cell_y = tf.transpose(cell_x, (0, 2, 1, 3, 4))

            cell_grid = tf.tile(tf.concat([cell_x, cell_y], -1), [1, 1, 1, self.nb_box, 1])
            ...
            """
            ### adjust x and y
            pred_box_xy = tf.sigmoid(tf.clip_by_value(y_pred[..., :2], 1.18e-38, float('inf'))) + cell_grid[:, :grid_h,
                                                                                                  :grid_w, :, :]

            ### adjust w and h
            pred_box_wh = tf.exp(y_pred[..., 2:4]) * np.reshape(self.anchors, [1, 1, 1, self.nb_box, 2])
            # pred_box_wh = tf.exp(tf.clip_by_value(y_pred[..., 2:4], 1.18e-38, float('inf'))) * np.reshape(self.anchors, [1, 1, 1, self.nb_box, 2])
